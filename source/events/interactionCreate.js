...
const { slash } = require(`${process.cwd()}/source/functions/onCoolDown.js`);
const { parsePermissions } = require(`${process.cwd()}/source/functions/functions.js`);
...
module.exports = {
  async execute(client) {
    ...
    client.on('interactionCreate', async interaction => {
      ...
      // Check if the interaction type is Autocomplete (4)
      ...
      try {
        ...
        if (slashCommand.toggleOff) {
          return await interaction.reply({
            ...
            embeds: [new EmbedBuilder()
              .setTitle(`${emojis.MESSAGE.x} **That Command Has Been Disabled By The Developers! Please Try Later.**`)
        ...
        if (slashCommand.ownerOnly) {
          ...
          if (!owners.includes(interaction.user.id)) {
            return await interaction.reply({
              ephemeral: true,
              embeds: [new EmbedBuilder()
                .setDescription(`${emojis.MESSAGE.x} **You cannot use \`${slashCommand.name}\` command as this is a developer command.**`)
                .setColor(client.embed.wrongcolor)
              ]
            }).catch(console.error);
          }
            ...
            return interaction.reply({
              ...
              embeds: [
                new EmbedBuilder()
                  ...
                  .setDescription(`The command \`${slashCommand.name}\` can only be used in the official server.`)
                  .setColor(client.embed.wrongcolor)
              ]
            });
          }
        ...
        if (client.config.CHANNELS.COMMANDS_LOGS && set.COMMANDS_LOGS) {
          await client.channels.cache.get(client.config.CHANNELS.COMMANDS_LOGS).send({
            embeds: [new EmbedBuilder()
